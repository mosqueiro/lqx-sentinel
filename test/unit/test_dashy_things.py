...
def test_valid_lqx_address():
    from lqxlib import is_valid_lqx_address

    main = valid_lqx_address()
    test = valid_lqx_address('testnet')

    assert is_valid_lqx_address(main) is True
    assert is_valid_lqx_address(main, 'mainnet') is True
    assert is_valid_lqx_address(main, 'testnet') is False

    assert is_valid_lqx_address(test) is False
    assert is_valid_lqx_address(test, 'mainnet') is False
    assert is_valid_lqx_address(test, 'testnet') is True


def test_invalid_lqx_address():
    from lqxlib import is_valid_lqx_address

    main = invalid_lqx_address()
    test = invalid_lqx_address('testnet')

    assert is_valid_lqx_address(main) is False
    assert is_valid_lqx_address(main, 'mainnet') is False
    assert is_valid_lqx_address(main, 'testnet') is False

    assert is_valid_lqx_address(test) is False
    assert is_valid_lqx_address(test, 'mainnet') is False
    assert is_valid_lqx_address(test, 'testnet') is False
...
def test_parse_masternode_status_vin():
    from lqxlib import parse_masternode_status_vin
...
def test_hash_function():
    ...
    hex_hash = "%x" % lqxlib.hashit(sb_data_hex)
...
def test_blocks_to_seconds():
    ...
    assert Decimal(lqxlib.blocks_to_seconds(0)) == Decimal(0.0)
    assert Decimal(lqxlib.blocks_to_seconds(2)).quantize(precision) \
        == Decimal(314.4).quantize(precision)
    assert int(lqxlib.blocks_to_seconds(16616)) == 2612035
