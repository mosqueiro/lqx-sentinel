...
sentinel_cfg = LqxConfig.tokenize(sentinel_config_file)
sentinel_version = "1.4.0"


def get_lqx_conf():
    if sys.platform == 'win32':
        lqx_conf = os.path.join(os.getenv('APPDATA'), "LqxCore/lqx.conf")
    else:
        home = os.environ.get('HOME')

        lqx_conf = os.path.join(home, ".lqxcore/lqx.conf")
        if sys.platform == 'darwin':
            lqx_conf = os.path.join(home, "Library/Application Support/LqxCore/lqx.conf")

    lqx_conf = sentinel_cfg.get('lqx_conf', lqx_conf)
