...
class LqxDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some lqxd methods
    ...
    @classmethod
    def from_lqx_conf(self, lqx_dot_conf):
        from lqx_config import LqxConfig
        config_text = LqxConfig.slurp_config_file(lqx_dot_conf)
        creds = LqxConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = lqxlib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = lqxlib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = lqxlib.blocks_to_seconds(diff)
