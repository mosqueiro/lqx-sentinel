...
def is_valid_lqx_address(address, network='mainnet'):
    ...
    lqx_version = 140 if network == 'testnet' else 76
    ...
    try:
        ...
        # rescue from exception, not a valid Lqx address
        return False

    if (address_version != lqx_version):
        return False
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('lqx.conf')
