...
class LqxConfig():
    ...
    def slurp_config_file(self, filename):
        # read lqx.conf config but skip commented lines
        f = io.open(filename)
        lines = []
        for line in f:
        ...
        # data is lqx.conf without commented lines
        data = ''.join(lines)
    ...
    def get_rpc_creds(self, data, network='mainnet'):
        # get rpc info from lqx.conf
        match = re.findall(r'rpc(user|password|port)=(.*?)$', data, re.MULTILINE)

        # python >= 2.7
        creds = {key: value for (key, value) in match}

        # standard Lqx defaults...
        default_port = 5784 if (network == 'mainnet') else 15784

        # use default port for network if not specified in lqx.conf
        if not ('port' in creds):
            creds[u'port'] = default_port

        # convert to an int if taken from lqx.conf
