...
def has_lqx_conf():
    ...
    valid_lqx_conf = False

    # ensure lqx_conf exists & readable
    #
    # if not, print a message stating that Lqx Core must be installed and
    # configured, including JSONRPC access in lqx.conf
    try:
        f = io.open(config.lqx_conf)
        valid_lqx_conf = True
    except IOError as e:
        print(e)

    return valid_lqx_conf
...
def main():
    ...
    if not has_lqx_conf():
        print("LqxCore must be installed and configured, including JSONRPC access in lqx.conf")
        sys.exit(1)
