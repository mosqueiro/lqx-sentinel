...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, lqxd):
        golist = lqxd.rpc_command('gobject', 'list')
        ...
        for item in golist.values():
            try:
                (go, subobj) = self.import_gobject_from_lqxd(lqxd, item)
            except Exception as e:
    ...
    @classmethod
    def import_gobject_from_lqxd(self, lqxd, rec):
        import decimal
        import lqxlib
        import binascii
        import gobject_json
        ...
        # exclude any invalid model data from lqxd...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from lqxd, with every run
        ...
        # get/create, then sync payment amounts, etc. from lqxd - Lqxd is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(lqxd)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from lqxd! %s" % e)
            govobj.vote_delete(lqxd)
            return (govobj, None)
    ...
    def vote_delete(self, lqxd):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(lqxd, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, lqxd, signal, outcome):
        import lqxlib
        ...
        output = lqxd.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = lqxlib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(lqxd, signal)

    def sync_network_vote(self, lqxd, signal):
        printdbg('\tSyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = lqxd.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = LQXD_GOVOBJ_TYPES['proposal']
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 lqx addr, non-multisig
            if not lqxlib.is_valid_lqx_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Lqx address for network [%s], returning False" % (self.payment_address, config.network))
                return False
            ...
            # Lqx Core restricts proposals to 512 bytes max
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (lqxlib.blocks_to_seconds(superblockcycle) / 2) +
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = LQXD_GOVOBJ_TYPES['superblock']
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not lqxlib.is_valid_lqx_address(addr, config.network):
    ...
    def hash(self):
        import lqxlib
        return lqxlib.hashit(self.serialise())
